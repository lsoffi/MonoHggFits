...
----------------------------------------------------------------------------------------------------------------------------------\n""" % signame)
    ...
    def generateSignalDataset(self,options,args):
        ...
        for signame in signals:
            ...
            for cat in fit["categories"]:
                ...
                treenameBef = "%s_%s_%s" % (signame,options.fit_name,cat)
                 ## dset = self.rooData(treename)
              
                #dset = self.rooData(treename,weight="%s * weight" % self.lumiScale(treename)) #Livia: i comment this because our signal are already scaled by the lumi
                dset = self.rooData(treename,weight="weight",redo=True)
    ...
    def plotBkgFit(self,options,dset,pdf,obs,label,blabel=None,extra=None,bias_funcs=None,poissonErrs=True,plot_binning=None,logx=True,logy=True,
        ...
        if binning:
            ...
            frameUnblind = obs.frame(RooFit.Range("plotBinning"))
        ...
        #dsetUnblind.plotOn(frameUnblind,*(dataopts))
       ...
       # dsetAll.plotOn(frameAll,*(dataopts+invisible)) 
        ...
        if sig:
            ...
            ymin = 0.0001 #fitc.interpolate(frame.GetXaxis().GetXmax())*0.25
        if not logy:
            ymax = ymax * 0.5
            ymin = 0.0001  #min(-0.02*ymax,ymin)
        ...
        if not sig: 
            frame.Draw()
        if doBlind and not sig:
            frameBlind.Draw("same")
        if not doBlind and not sig:
            frameUnblind.Draw("same")
        #if sig:
        #    frameSig.Draw()
