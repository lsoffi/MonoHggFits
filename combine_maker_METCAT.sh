...
input=${version}_${fitname}_final_ws_METCAT.root
input_log=${version}_${fitname}_final_ws_METCAT.log
treesdir=inputData
## ls $treesdir/$version
...
if [[ -n $rerun  ]] || [[ ! -f $input ]]; then
    ...
    ./templates_maker_METCAT.py --load templates_maker_METCAT.json,templates_maker_prepare_METCAT.json --only-subset $subset $mix --input-dir $treesdir/$input_folder -o $input $verbose $input_opts 2>&1 | tee $input_log
    echo "**************************************************************************************************************************"
elif [[ -n $mix ]]; then
    echo "**************************************************************************************************************************"
    echo "running event mixing"
    echo "**************************************************************************************************************************"    
    ./templates_maker_METCAT.py --load templates_maker_prepare_METCAT.json --read-ws $input $mix $verbose 2>&1 | tee mix_$input_log
    echo "**************************************************************************************************************************"
fi
...
./combine_maker_METCAT.py \
    ...
    --read-ws $input \
    --ws-dir $workdir \
    -O $www/$version/${workdir}_METCAT \
    -o ${workdir}_METCAT.root  \
   # --cardname datacard_${workdir}_METCAT.txt $opts 2>&1 | tee $workdir/combine_maker${log_label}_METCAT.log
else
./combine_maker_METCAT.py \
    --fit-name $fitname  --luminosity $lumi  --lumi $lumi \
    --fit-background \
