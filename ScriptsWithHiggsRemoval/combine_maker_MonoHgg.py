            ...
            option_groups=[
                ("Combine workspace options", [
                        ...
                        make_option("--observables",dest="observables",action="callback",callback=optpars_utils.Load(scratch=True),type="string",
                                    default={ "EBEE" : "mggEBEE[75,105,180]",
                                              "EBEB" : "mggEBEB[75,105,180]"
                                              },
                        ...
                        make_option("--plot-blind-data",dest="plot_blinded_data",action="store_true",default=False,
                                    help="Blind signal region in data",
                                    ),
                        ...
                        make_option("--generate-signal-dataset",dest="generate_signal_dataset",action="store_true",default=True,
...
----------------------------------------------------------------------------------------------------------------------------------\n""" % signame)
            ...
            datacard.write("hgg_BR  lnN".ljust(20))
            for cat in categories:
                datacard.write(" 0.953/1.050".ljust(15) )
                if options.generate_higgs_dataset:
                    datacard.write(" 0.953/1.050".ljust(15) )
                    datacard.write(" 0.953/1.050".ljust(15) )
                    datacard.write(" 0.953/1.050".ljust(15) )
            ...
            datacard.write("hgg_alphaSWeight_0  lnN".ljust(20))
            for cat in categories:
                datacard.write(" 0.940/0.965".ljust(15) )             # taken from Hgg, untagged2, ggH
                if options.generate_higgs_dataset:
                    datacard.write(" 0.940/0.965".ljust(15) )             # taken from Hgg, untagged2
                    datacard.write(" 0.970/0.978".ljust(15) )             # taken from Hgg, untagged2 
                    datacard.write(" 0.974/1.008".ljust(15) )             # taken from Hgg, untagged2  
                    datacard.write(" 0.987/1.003".ljust(15) )             # taken from Hgg, untagged2  
                for comp in options.components:
                    datacard.write(" -".ljust(15) )
            datacard.write("\n")       
    ...
    def generateSignalDataset(self,options,args):
        ...
        for signame in signals:
            ...
            for cat in fit["categories"]:
                ...
                #    print options.signal_scalefactor_forpdf
                #    print "insie"
                ...
                if options.plot_signal_binning:
                    ...
                    plot_signal_binning = []
                    while omin<omax:
                        plot_signal_binning.append(omin)
                        omin += step                    
                ...
                self.plotBkgFit(options,reduced,pdf,roobs,"signal_%s_%s_%s" % (signame,roobs.GetName(),cat),poissonErrs=False,sig=True,hig=False,logx=False,logy=False,
                                plot_binning=plot_signal_binning, forceSkipBands=True)
    ...
    def generateHiggsDataset(self,options,args):
        ...
        for higname in higgs:
            ...
            for cat in fit["categories"]:
                ...
                self.plotBkgFit(options,reduced,pdf,roobs,"model_higgs_%s_%s_%s" % (higname,roobs.GetName(),cat),poissonErrs=False,sig=True,hig=True,logx=False,logy=False,
    ...
    def plotBkgFit(self,options,dset,pdf,obs,label,blabel=None,extra=None,bias_funcs=None,poissonErrs=True,plot_binning=None,logx=True,logy=True,
                       opts=[],forceSkipBands=False,sig=False, hig=False):
        ## plot the fit result
        if hig:
            sig=hig
        ...
        if binning:
            ...
            frameAll = obs.frame(RooFit.Range("plotBinning"))
            frameBlind = obs.frame(RooFit.Range("plotBinning"))
            frameSig = obs.frame()#RooFit.Range("plotBinning")
            resid  = obs.frame(RooFit.Range("plotBinning"))
            residSig  = obs.frame()
            residDown  = obs.frame(RooFit.Range("plotBinning"))
            residUp  = obs.frame(RooFit.Range("plotBinning"))
            ## curveopts.append(RooFit.NormRange("fullRange"))
            ## dataopts.append(RooFit.Range("plotBinning"))
        else:
            frame = obs.frame()          
            frameAll = obs.frame()          
            frameBlind = obs.frame()
            frameSig = obs.frame()
            residSig  = obs.frame()
            resid  = obs.frame()
            residDown  = obs.frame()
        ...
        doBlind=options.plot_blinded_data
        doBlind=True
        if doBlind:
            print "blind data in 115=135 GeV"
        
        dsetAll=dset.reduce("mgg>0")
        if doBlind:
            dsetDown=dset.reduce("mgg<115")
            dsetDown.SetName("dsetDown")
            dsetUp=dset.reduce("mgg>135")
            dsetUp.SetName("dsetUp")
        ...
        print dset.sumEntries()
        pdf.plotOn(frame,*(curveopts+invisible))       
       
        if doBlind and not sig:
            dset.plotOn(frameBlind,RooFit.Invisible())
            pdf.plotOn(frameBlind,*(curveopts+invisible))              
            dsetDown.plotOn(frameBlind,*(dataopts))
            dsetUp.plotOn(frameBlind,*(dataopts)) 
           
    
       # dsetAll.plotOn(frameAll,*(dataopts+invisible)) 
        
        #signal plotting
        if sig:
            obs.setRange("range",100, 180)
        ...
        if doBlind and not sig:
            histUp   = frameBlind.getObject(int(frameBlind.numItems()-1))
            histDown   = frameBlind.getObject(int(frameBlind.numItems()-1))
       
      
        fitcLeg,histLeg = fitc,hist
        ## print hist, fitcRooFit.Invisible()
        if extra:
        ...
        if doBlind and not sig:
            hresidUp=frameBlind.residHist(histUp.GetName(),frame.getObject(int(frame.numItems()-1)).GetName(),True)
            residUp.addPlotable(hresidUp,"PE")
            residUp.addObject(one)
            hresidDown=frameBlind.residHist(frameBlind.getObject(int(frameBlind.numItems()-2)).GetName(),frame.getObject(int(frame.numItems()-1)).GetName(),True)
            residDown.addPlotable(hresidDown,"PE")

        if sig and not hig: 
            canv = ROOT.TCanvas("sig_fit_%s" % label, "sig_fit_%s" % label)
            legend = None
        if  hig: 
            canv = ROOT.TCanvas("hig_fit_%s" % label, "sig_fit_%s" % label)
            legend = None
        if not sig and not hig:
        ...
        frameSig.GetXaxis().SetLabelSize( 1.5*frameSig.GetXaxis().GetLabelSize() )
        frameSig.GetXaxis().SetTitleSize( 1.5*frameSig.GetXaxis().GetTitleSize() )
        frameSig.GetXaxis().SetTitleOffset( 0.6 )
        #frameSig.GetYaxis().SetRangeUser(ymin,ymax)
        #frameSig.GetXaxis().SetMoreLogLabels()
        frameSig.GetYaxis().SetLabelSize( frameSig.GetXaxis().GetLabelSize() * canv.GetWh() / ROOT.gPad.GetWh() * 0.9 )
        frameSig.GetYaxis().SetTitleSize( frameSig.GetXaxis().GetTitleSize() * canv.GetWh() / ROOT.gPad.GetWh() )
        frameSig.GetYaxis().SetTitleOffset( 0.6 )
        
        if not logy:
            frameSig.GetYaxis().SetNdivisions(505)
        frameSig.GetXaxis().SetNdivisions(510)
        frame.GetYaxis().SetRangeUser(0.001, 10000)
        if not sig: 
            frame.Draw()
        
        if doBlind and not sig:
            frameBlind.Draw()
        #if not doBlind and not sig:
            #frameAll.Draw("same")
        #if sig:
        #    frameSig.Draw()
        ...
        resid.GetXaxis().SetRangeUser( 100., 180. )
        resid.Draw()
        if sig:
            residSig.GetYaxis().SetNdivisions(505)
            residSig.GetYaxis().CenterTitle()
            residSig.GetYaxis().SetTitleSize( frame.GetYaxis().GetTitleSize() * 6.5/3.5 )
            residSig.GetYaxis().SetTitleOffset( frame.GetYaxis().GetTitleOffset() * 3.5/6.5 ) # not clear why the ratio should be upside down, but it does
            residSig.GetYaxis().SetLabelSize( frame.GetYaxis().GetLabelSize() * 6.5/3.5 )
            residSig.GetXaxis().SetTitleSize( frame.GetXaxis().GetTitleSize() * 6.5/3.5 )
            residSig.GetXaxis().SetTitleOffset( frame.GetXaxis().GetTitleOffset() )
            residSig.GetXaxis().SetLabelSize( frame.GetXaxis().GetLabelSize() * 6.5/3.5 )
            residSig.GetYaxis().SetTitle("(data-fit)/#sigma_{data}")
            residSig.GetYaxis().SetRangeUser( -5., 5. )
            residSig.GetXaxis().SetRangeUser( 100., 180. )
            
            residSig.Draw()
       
        if doBlind and not sig:
            resid.GetYaxis().SetNdivisions(505)
            residUp.GetYaxis().CenterTitle()
            residUp.GetYaxis().SetTitleSize( frame.GetYaxis().GetTitleSize() * 6.5/3.5 )
            residUp.GetYaxis().SetTitleOffset( frame.GetYaxis().GetTitleOffset() * 3.5/6.5 ) # not clear why the ratio should be upside down, but it does
            residUp.GetYaxis().SetLabelSize( frame.GetYaxis().GetLabelSize() * 6.5/3.5 )
            residUp.GetXaxis().SetTitleSize( frame.GetXaxis().GetTitleSize() * 6.5/3.5 )
            residUp.GetXaxis().SetTitleOffset( frame.GetXaxis().GetTitleOffset() )
            residUp.GetXaxis().SetLabelSize( frame.GetXaxis().GetLabelSize() * 6.5/3.5 )
            residUp.GetYaxis().SetTitle("(data-fit)/#sigma_{data}")
            residUp.GetYaxis().SetRangeUser( -5., 5. )
            residUp.Draw()
            residDown.Draw("same")
